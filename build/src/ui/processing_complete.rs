...
for addr in range(1, 10):
    try:
        ...
        # Send Modbus read device ID
        msg = struct.pack('>HHHBB', 0, 0, 6, addr, 0x2B)
        sock.send(msg)
        response = sock.recv(1024)
        if response:
            print(f'{addr}:Device_{addr}:Online')
        sock.close()
    except:
        pass
")
                ...
                for line in String::from_utf8_lossy(&output.stdout).lines() {
                    ...
                    if parts.len() >= 3 {
                        devices.push(DiscoveredDevice {
                            device_id: format!("MB{:02}", parts[0]),
                            device_name: parts[1].to_string(),
                            manufacturer: "Sequent".to_string(),
                            model: "MegaBAS".to_string(),
            ...
            if let Ok(output) = result {
                for line in String::from_utf8_lossy(&output.stdout).lines() {
                    if line.contains("Device") {
                        // Parse BACnet device info
                        devices.push(DiscoveredDevice {
                            device_id: format!("BAC{}", devices.len()),
                            device_name: line.to_string(),
                            manufacturer: "Unknown".to_string(),
                            model: "BACnet Device".to_string(),
        ...
        // Also check for local Sequent boards
        let boards_result = std::process::Command::new("python3")
            .arg("/opt/nexus/scripts/megabas_interface.py")
            .arg("scan")
            .output();
        
        if let Ok(output) = boards_result {
            for line in String::from_utf8_lossy(&output.stdout).lines() {
                if line.contains("Found") {
                    devices.push(DiscoveredDevice {
                        device_id: format!("SEQ{}", devices.len()),
                        device_name: line.to_string(),
                        manufacturer: "Sequent".to_string(),
                        model: "Hardware Board".to_string(),
                        address: "I2C".to_string(),
                        points_count: 8,
                        online: true,
                    });
                }
    ...
    fn test_read_point(&mut self) {
        // Read REAL point value from hardware
        if let Some(test_point) = &self.test_point {
            let parts: Vec<&str> = test_point.split('.').collect();
            if parts.len() >= 2 {
                let device = parts[0];
                let point = parts[1];
                
                // Read from MegaBAS if it's a local device
                if device.starts_with("MB") || device.starts_with("SEQ") {
                    ...
                    if let Ok(output) = result {
                        if let Ok(value) = String::from_utf8_lossy(&output.stdout).trim().parse::<f32>() {
                            self.test_result = Some(PointValue::Float(value * 10.0)); // Scale 0-10V
                        }
                    }
                } else if device.starts_with("BAC") {
                    // Read from BACnet device
                    let result = std::process::Command::new("bacnet-read")
                        .args(&[device, point])
                        .output();
                    
                    if let Ok(output) = result {
                        if let Ok(value) = String::from_utf8_lossy(&output.stdout).trim().parse::<f32>() {
                            self.test_result = Some(PointValue::Float(value));
                        }
                    }
                }
            }
