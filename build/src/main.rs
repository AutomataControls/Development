...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Start API server in background (for direct API access)
    ...
    // Start Web UI server for remote access via Cloudflare
    let web_state = app_state.clone();
    tokio::spawn(async move {
        info!("Starting web UI server on port 1420 for Cloudflare tunnel");
        web_server::start_web_server(web_state).await;
    });
