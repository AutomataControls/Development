...
module.exports = chillerPlant;"#.to_string()
    ...
    fn execute_logic(&mut self, logic_id: &str) {
        ...
        if let Some(file) = file {
            let input_channel = file.input_channel;
            let current_temp = self.universal_inputs[input_channel];
            
            // Create inputs
            let mut inputs = HashMap::new();
            inputs.insert("currentTemp".to_string(), current_temp);
            inputs.insert("targetTemp".to_string(), self.temp_control_settings.target_temp);
            inputs.insert("supplyTemp".to_string(), self.universal_inputs[0]);
            inputs.insert("returnTemp".to_string(), self.universal_inputs[1]);
            inputs.insert("outdoorTemp".to_string(), self.universal_inputs[5]);
            inputs.insert("staticPressure".to_string(), self.universal_inputs[3]);
            
            // Simulate outputs based on temperature error
