...
impl RefrigerantDiagnostics {
                ...
                for (label, key, value) in temp_configs {
                    ...
                    if config.enabled {
                        ...
                        if let Some(channel) = &mut config.input_channel {
                            ui.add(egui::DragValue::new(channel).speed(1).clamp_range(0..=15));
                        } else {
                            ...
                            if circuit.suction.enabled {
                                ui.horizontal(|ui| {
                                    ...
                                    if let Some(channel) = &mut circuit.suction.input_channel {
                                        ui.add(egui::DragValue::new(channel).speed(1).clamp_range(0..=7));
                                    }
    ...
    fn update_monitoring_data(&mut self) {
        // Simulate reading from transducers and updating diagnostics
        for (circuit_id, circuit) in &self.circuits {
            if !circuit.enabled {
                continue;
            }
            
            // Simulate pressure readings with realistic variations
            let pressures = self.circuit_pressures.get_mut(circuit_id).unwrap();
            pressures.suction = 68.0 + (rand::random::<f32>() - 0.5) * 10.0;
            pressures.discharge = 325.0 + (rand::random::<f32>() - 0.5) * 30.0;
